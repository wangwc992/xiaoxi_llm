    ...
    if query_type == TASK_A or query_type == TASK_B:
        ...
        if not service_master_list:
            ...
            if task == "14":
                ...
                classification_model.ids = output_dict.get("ids")
...
async def classification(ai_chat_log_model: AiChatLogModel, reanswer: bool,
    ...
    # 创建chat_completion请求
    result = completions(chat_request)
    # 提取消息
    chat_completion_stream_response = await extract_message(result)
...
async def stream_response(chat_request,
    ...
    async for chunk in create_chat_completion(chat_request):
        ...
        if first_chunk:
            ...
            chat_completion_stream_response.classification_model = classification_model.model_dump()
...
async def chat_responsr_to_chat_log(ai_chat_log_model, chat_completion_stream_response):
    ...
    ai_chat_log_model.chat_id = chat_completion_stream_response.id
    logger.info("非流回复提取内容： %s", chat_completion_stream_response.model_dump_json())
