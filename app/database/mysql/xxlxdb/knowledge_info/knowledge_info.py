...
def search_missing_knowledge_info_data(db_id_list: list):
    ...
    sql = f'''
        SELECT 
            id, type, country, school, class, name, founder,filename, updateTime, content, fileurl,`class` as class_ 
        FROM 
            t_knowledge_info
        WHERE 
...
def search_knowledge_info_data(id=0, limit=10):
    ...
    if limit == 1:
        ...
        pj = f"> {id}"
    sql = f"""
        SELECT t1.id, type, country, school, class, name, founder,filename, updateTime, content, fileurl,`class` as class_ ,amw.output AS output
        FROM t_knowledge_info t1
